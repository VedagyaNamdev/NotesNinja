...
interface FlashcardNoteSelectorProps {
  onNoteSelected: (content: string, title: string) => void;
}

export default function FlashcardNoteSelector({ onNoteSelected }: FlashcardNoteSelectorProps) {
        ...
        if (isDocument) {
          ...
          try {
            ...
            toast.success(`Document content extracted: ${note.title}`, {
              description: 'Document text has been extracted for flashcard generation'
            });
          } catch (extractError) {
      ...
      <CardContent className="p-4">
        <div className="flex flex-col space-y-4">
          ...
          {loading ? (
            ...
            <div className="py-2 text-center text-sm text-muted-foreground">
              No notes found. Create some notes to generate flashcards.
            </div>
          )}
